<x-app-layout>
    ...
    @endif
        ...
        <div class="max-w-lg mx-auto bg-custom text-white p-6 rounded-lg shadow-lg mt-8">
            <form id="myForm" action="{{ route('divisions.store') }}" method="POST">
                @csrf
                <div class="form-group mb-4">
                    <label for="name" class="block text-black font-bold mb-2">Nama:</label>
                    <input type="text" name="name" class="text-black form-control w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500" value="{{ old('nama', $kategorimasalah->nama ?? '') }}" required>
                </div>
                <div class="form-group mb-4">
                    <label for="status" class="block text-black font-bold mb-2">Status:</label>
                    <select for="status" class="text-black" required name="status" required>
