...
@endif
    ...
    <div class="max-w-lg mx-auto bg-custom text-white p-6 rounded-lg shadow-lg mt-10">
        <form action="{{ route('kategorimasalah.update', $kategorimasalah->id) }}" method="POST">
            @csrf
            @method('PUT')
                <div class="mb-2">
                ...
                <div class="mb-2">
                    <label for="divisions_id" class="block text-gray-700 light:text-gray-300">Division</label>
                    <select name="divisions_id" class="text-black w-full px-4 py-2 border border-cyan-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-cyan-500" required>
                        @foreach ($divisions as $division)
                            <option class="block text-gray-700 light:text-gray-300" value="{{ $division->id }}" {{ (old('division_id ', $kategorimasalah->division_id ?? '') == $division->id) ? 'selected' : '' }}>
                                {{ $division->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-2">
                    <label for="status" class="block text-gray-700 light:text-gray-300">Status:</label>
                    <input type="text" name="status" class="text-black w-full px-4 py-2 border border-cyan-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-cyan-500" value="{{ old('status', $kategorimasalah->status ?? 'active') }}" required>
                </div>
