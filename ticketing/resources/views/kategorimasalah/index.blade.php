        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="mb-6">
                <p class="mb-2 text-lg text-black-800 dark:text-gray-500">
                    Tambahkan Katego/ri baru dengan mengklik tombol di bawah ini:
                </p>
                <a href="{{ route('kategorimasalah.create') }}" class="btn custom-button-color inline-block bg-blue-400 hover:bg-blue-600 text-white font-bold py-1 px-6 rounded">
                    Add New
                </a>
            </div>
            <div class="overflow-x-auto mt-6"> <!-- Added margin-top to create space between button and table -->
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-800 bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-200">
                    <thead class="table-header dark:table-header-dark">
                        <tr>
                            <th class="px-6 py-2 border-b-2 border-gray-200 dark:border-gray-700 text-left">ID Kategori</th>
                            <th class="px-6 py-2 border-b-2 border-gray-200 dark:border-gray-700 text-left">Kategori</th>
                            <th class="px-6 py-2 border-b-2 border-gray-200 dark:border-gray-700 text-left">Division</th>
                            <th class="px-6 py-2 border-b-2 border-gray-200 dark:border-gray-700 text-left">Status</th>
                            <th class="px-6 py-2 border-b-2 border-gray-200 dark:border-gray-700 text-left">Actions</th>
                        </tr>
                    </thead>
                    <tbody>
                        @foreach ($kategorimasalah as $kategorimasalah)
                            <tr class="table-row dark:table-row-dark table-row-hover dark:table-row-dark-hover">
                                <td class="px-6 py-4">{{ $kategorimasalah->id }}</td>
                                <td class="px-6 py-4">{{ $kategorimasalah->nama }}</td>
                                <td class="px-6 py-4">{{ $kategorimasalah->divisions->name }}</td>
                                <td class="px-6 py-4">{{ $kategorimasalah->status }}</td>
                                <td class="px-6 py-4">
                                    <a href="{{ route('kategorimasalah.edit', $kategorimasalah->id) }}" class="text-black hover:text-black">Edit</a>
                                    <form action="{{ route('kategorimasalah.destroy', $kategorimasalah) }}" method="post" style="display:inline-block;">
                                        @csrf
                                        @method('DELETE')
